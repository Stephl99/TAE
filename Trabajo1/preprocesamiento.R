...
# Cambiar nombres de variables y vovler data frame

datos$conyuges <- as.factor(datos$conyuges[,1])
datos$padre_hogar <- as.factor(datos$padre_hogar[,1])
datos$educacion_padre <- as.factor(datos$educacion_padre[,1])
datos$madre_hogar <- as.factor(datos$madre_hogar[,1])
datos$educacion_madre <- as.factor(datos$educacion_madre[,1])
datos$cultura <- as.factor(datos$cultura[,1])
datos$es_campesino <- as.factor(datos$es_campesino[,1])
datos$condicion_vida <- as.factor(datos$condicion_vida[,1])
datos$region <- as.factor(datos$region[,1])
datos$tipo_vivienda <- as.factor(datos$tipo_vivienda[,1])
datos$energia <- as.factor(datos$energia[,1])
datos$estrato <- as.factor(datos$estrato[,1])
datos$acueducto <- as.factor(datos$acueducto[,1])
datos$alcantarillado <- as.factor(datos$alcantarillado[,1])
datos$vivienda_ocupada <- as.factor(datos$vivienda_ocupada[,1])
datos$fisico_t <- as.factor(datos$fisico_t[,1])
datos$intelectual_t <- as.factor(datos$intelectual_t[,1])

datos$arriendo <- as.numeric(datos$arriendo[,1])
datos$num_computadores <- as.numeric(datos$num_computadores[,1])


datos <- as.data.frame(datos)

# Cambio de nombres
datos <- datos %>% rename(corte_energia_fp = P5016S5,
                          corte_energia_om = P5016S6,
                          bajo_voltaje = P5016S3,
                          ninguna = P5016S4,
                          gas_natural_rp = P5666,
                          tipo_serv_sanitario = P8526,
                          ubic_serv_sanitario = P5022,
                          serv_sanitario_es = P5030,
                          lavamanos = P1892S1,
                          lavadero = P1892S2,
                          lavaplatos = P1892S3,
                          ninguno = P1892S4,
                          jabon_manos = P1893,
                          basura_hogar = P5041,
                          clasifica_baura = P5046,
                          bombilla_bajo_consumo= P5012S1,
                          apaga_luces=P5012S2,
                          planchar=P5012S3,
                          desconecta_aparatos = P5012S8,
                          reutiliza_agua=P5012S4,
                          recolecta_agua_lluvia=P5012S5,
                          tanque_sanitario_bj =  P5012S6,
                          econom_agua = P5012S7,
                          obtencion_agua_alimento = P8530,
                          agua_24h = P5047,
                          obtencion_agua_bebida=P5069,
                          ubic_prepar_alimento=P764,
                          tipo_energia = P8536)
datos <- datos %>% rename(cambios_voltaje = P5016S2,
                          ilum_ppal = P3000)
# Exportar datos --------
