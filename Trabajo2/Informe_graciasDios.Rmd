...
```{r}
promedio_entrantes_tx[is.na(promedio_entrantes_tx)] <- 0
evaluacion <- promedio_entrantes_tx
resultado <- segmentar_columna(promedio_entrantes_tx, seq(0.1, 1, by= 0.1))
(cuantiles <- quantile(promedio_entrantes_tx, seq(0.1, 1, by= 0.1)))
evaluar <- data.frame(resultado, promedio_entrantes_tx)
```
...
```{r}
caracteristicas[1:4] <- apply(caracteristicas[1:4], MARGIN = 2, segmentar_columna, percentiles = seq(0.1, 1, by = 0.1))
```
...
caracteristicas <- cbind(caracteristicas, base[nombres_categoricas], pagos_pn,
                         tiene_ventas, recaudos)
```



```{r}
correlaciones_caracteristicas <- cor(caracteristicas)
corrplot(correlaciones_caracteristicas)
```

```{r}
componentes <- prcomp(caracteristicas)
...
```{r, fig.align='center', out.width='70%}
set.seed(280721)
kmeans <- kmeans(caracteristicas, 5, iter.max =1000, nstart = 10)
clusters <- kmeans$cluster
ggbiplot(componentes, groups = factor(clusters ) )
```
...
table(grupo5$promedio_salientes_tx)
```
```{r}
...
table(grupo5$promedio_entrantes_vm)
```
```{r}
...
table(grupo5$promedio_salientes_vm)
```
```{r}
...
```{r}
valores <- componentes$x
valores <- as.data.frame(valores)
valores$clusters <- clusters
ggplot(valores, aes(x = PC1, y = PC2, col = as.factor(clusters)) ) + geom_point()
```
